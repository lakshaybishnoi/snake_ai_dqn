...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        if self.head == self.food:
            ...
            # Add small reward for moving closer to food
    ...
    def _is_closer_to_food(self):
        # Check if the snake is moving closer to the food
        current_distance = abs(self.head.x - self.food.x) + abs(self.head.y - self.food.y)
        if len(self.snake) > 1:
            previous_head = self.snake[1]
            previous_distance = abs(previous_head.x - self.food.x) + abs(previous_head.y - self.food.y)
    ...
    def _update_ui(self):
        ...
        for pt in self.snake:
            ...
            pygame.draw.rect(self.display, pygame.Color('darkgreen'), pygame.Rect(pt.x + 4, pt.y + 4, 12, 12))
